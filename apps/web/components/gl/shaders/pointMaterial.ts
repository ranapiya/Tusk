  ...
  constructor() {
    super({
      ...
      void main() {
        ...
        vec3 blueColor = vec3(0.2, 0.4, 1.0); // Bright blue
        vec3 purpleColor = vec3(0.6, 0.2, 0.8); // Purple
        vec3 pinkColor = vec3(0.9, 0.3, 0.7); // Pink accent
        
        // Create gradient based on Y position and world position
        float gradientFactor = (vWorldPosition.y + 2.0) / 4.0; // Normalize Y position
        gradientFactor = clamp(gradientFactor, 0.0, 1.0);
        
        // Mix blue to purple based on Y position
        vec3 baseColor = mix(blueColor, purpleColor, gradientFactor);
        
        // Add some pink accents based on X position for more variety
        float pinkFactor = sin(vWorldPosition.x * 0.5) * 0.5 + 0.5;
        baseColor = mix(baseColor, pinkColor, pinkFactor * 0.3);
        
        // Apply sparkle brightness to the color
        baseColor *= sparkleBrightness;
